...
while (<PORTLIST>) {
    ...
    $importargs="--name=$munkiname --pkgvers=$ver --subdirectory $BASE";
    $importargs.=" --minimum_os_version=10.8.0 --maximum_os_version=10.9.999";
	...
	$jobout=system("makepkginfo $importargs $portpkg > $INFOS/$portname-$portver.plist");
    } else {
	$jobout=system("munkiimport -n $importargs $portpkg");
    }
